...
def _apply_bleed_gate(monos: list, sr: int) -> list:
    ...
    hop_ms = 12.0
    silence_db = -60.0
    # Winner-takes-all: if a mic is sufficiently quieter, gate it hard.
    start_db = 0.5
    max_att_db = 60.0
    ...
    for idx in range(env_db.size(0)):
        ...
        g = _smooth_gain_envelope(g, attack_ms=10.0, release_ms=400.0, sr=sr, hop_ms=hop_ms)
        gains_lin.append(g)
    ...
    for mono, g in zip(padded, gains_lin):
        ...
        g_up = g_up[:T]
        # Additional smoothing directly on the upsampled gain to avoid rapid gain jumps
        filt_len = max(1, int(sr * 0.02))
        if filt_len > 1:
            kernel = torch.ones(filt_len, dtype=g_up.dtype, device=g_up.device) / float(filt_len)
            g_up = torch.nn.functional.conv1d(g_up.unsqueeze(0).unsqueeze(0), kernel.view(1, 1, -1), padding=filt_len//2).squeeze()
            g_up = g_up[:T]
